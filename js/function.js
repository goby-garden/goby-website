...
let goby = {
  ...
  fields: [{ key: "type", default: "entree" }],
  blocks: []
};
...
function fillMeta(data) {
  ...
  document
    ...
    .insertAdjacentHTML("beforeend", data.title);
...
let chanLines =
  '<line class="line" x1="0%" x2="100%" y1="0%" y2="100%"></line><line class="line" x1="100%" x2="0%" y1="0%" y2="100%"></line>';
    ...
    .each((d, i, nodes) => {
      ...
      if (d.image) {
        ...
        cNode
          .select(".text-block")
          .node()
          .insertAdjacentHTML("afterbegin", d["content_html"]);
        ...
        cNode
          .select("svg")
          .node()
          .insertAdjacentHTML("afterbegin", chanLines);
      }
      cNode
        .append("p")
        .attr("class", "block-title emphasis")
        .html(d.title);
...
function textAreaHeights(first) {
  document.querySelectorAll("textarea").forEach((item, i) => {
    item.setAttribute(
      "style",
      "height:" + item.scrollHeight + "px;overflow-y:hidden;"
    );
    if (first) {
...
function textAreaOnInput(node) {
  if (this.style !== undefined) {
    ...
    node.setAttribute(
      "style",
      "height:" + node.scrollHeight + "px;overflow-y:hidden;"
    );
...
function newTagAdd() {
  console.log(event);
  if (event.data == ",") {
    let newTagVal = document
      .querySelector("#add-new-tag")
      .value.replace(",", "");
